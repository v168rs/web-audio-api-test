...
//STRESS TEST - dump all of the countries in and see what happens
...
//Takes a path/URL to the sample and a function that receives the arraybuffer data as a parameter. Don't call this multiple times for multiple samples. Instead use multiple_xhr.
...
//XHR - url_array, responseType, receiver
function multiple_xhr (url_array, rt, receiver) {
    ...
    xhr.onload = function (b) {
        receiver(xhr.response, i); //wwew lad
        var n2 = url_iterator.next();
        i++;
        if(n2.done == false) {
...
var geo_buses = [];
var listening_buses = [];
var cur_audio_selector = "anthems";
var context;
var convolver;
...
//4000 km?
function find_prox_nodes(range = 2000000, audio_selector = "anthems", lat, long) {
    var ind_arr = [],
        url_arr = [],
        rej_arr = [];
    ...
    if(url_arr[0]) {
        multiple_xhr(url_arr, "arraybuffer", function(b, i) {
        context.decodeAudioData(b, function(d) {
            if(!(geo_buses[ind_arr[i]][0] instanceof AudioBufferSourceNode)) {
    ...
    rej_arr.forEach(function(index) {
        if(geo_buses[index][0] instanceof AudioBufferSourceNode) {
...
function set_samples_loc(arr, lat = 0, long = 0){ 
    var loc_lat = lat, //y
        loc_long = long; //x
        s_maj = 6378137,
...
camera.changed.addEventListener(function() {
    ...
    find_prox_nodes(1000000, cur_audio_selector, camera.positionCartographic["latitude"], camera.positionCartographic["longitude"]); //Europe is terrifying
                                           });
...
function create_samples_with_loc(audio_selector = "anthems", cone_inner = 10, cone_outer = 140){
    geo_audio_samples[audio_selector].forEach(function(arr){
        ...
        //Depending on how well this performs one may need to delete the sampler and reconnect another one to the panner based on distance
